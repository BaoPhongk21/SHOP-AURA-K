/**
 * restore-local.js - Khoi phuc CSDL PostgreSQL Local (localhost:5432)
 * Chay: cd D:\Shop-Quan-Ao - DELOY\server && node restore-local.js
 *
 * - Dung .env hien tai (localhost)
 * - Reset schema + seed du lieu mau
 */
...
// Tu .env - mac dinh localhost:5432 / shop-quan-ao / postgres / 123
const DB_URL = process.env.DATABASE_URL || 'postgres://postgres:123@localhost:5432/shop-quan-ao';
const SEED_PASSWORD = process.env.SEED_DEFAULT_PASSWORD || 'B1n123123@';
const ADMIN_EMAIL = process.env.SEED_ADMIN_EMAIL || 'levietbaophong111@gmail.com';
...
async function seedBasic() {
  ...
  await sequelize.query(
    ...
    { replacements: { adminEmail: ADMIN_EMAIL, hash: hashedPassword } }
  );
  console.log('   OK - 3 users (admin + 2 customer)');
...
async function main() {
  try {
    ...
    console.log('OK - Ket noi PostgreSQL LOCAL thanh cong\n');

    console.log('Kiem tra database hien tai...');
    const existing = await ensureTables();

    if (existing === 0) {
      console.log('\n-> DB trong, tao schema moi...');
      await createSchema();
    } else {
      console.log(`\n-> DB da co ${existing} bang, reset sach...`);
      await resetSchema();
      await createSchema();
    }
    ...
    console.log('\n========================================');
    console.log('HOAN TAT KHOI PHUC POSTGRESQL LOCAL!');
    console.log('Admin: ' + ADMIN_EMAIL + ' / admin / ' + SEED_PASSWORD);
    console.log('========================================');
